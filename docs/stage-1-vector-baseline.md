...
Exact, not approximate. The query — and only the query — carries the BGE prefix.
...
*Cost:* a definition spanning pages 12–13 gets cut in half. Unmeasured so far.
...
2,611 × 384 float32 is 4 MB — small enough to sit in CPU cache, where an
...
Output bills at 5× input. `max_tokens` is a genuine per-call spend cap. Adaptive
...
50–100 real questions with known-correct chunk IDs, **segmented by kind**
(identifier / conceptual / multi-hop / unanswerable), plus recall@k, MRR and
nDCG. This is the instrument, and it is the largest remaining piece — hours of
...
**4. Take the baseline measurement.** ← this closes stage 1
...
*and* RRF and recall jumps 15 points, you cannot say which did it — and "we added
