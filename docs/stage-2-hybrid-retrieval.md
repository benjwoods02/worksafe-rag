...
The channels produce incomparable scales — cosine sits in 0–1, BM25 is
...
**Query escaping.** FTS5 `MATCH` is a query language, not raw text — unescaped
...
Neither variant beats the baseline on `hit@5` or `nDCG@5`. **Rejected.**
...
contents-page lines). Body text is consistently 8.5 pt with headings at 10–14 pt
...
**A cheap proxy for retrieval quality is not a substitute for running the eval.**
